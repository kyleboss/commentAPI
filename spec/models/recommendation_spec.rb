describe Recommendation do
  ...
  describe '.all_for' do
    before(:each) do
      expect(Recommendation).to receive(:relevant_doctors_close_to).with(doctor) { [recommended_doctor] }
  ...
  describe '.doctors_close_to' do
    ...
    it 'returns the result of by_distance' do
      expect(Recommendation.send(:relevant_doctors_close_to, doctor)).to eq [recommended_doctor]
    end
  end
  ...
  describe '.sort_doctors_by_score' do
    ...
    context 'doctors is not empty' do
      ...
      before(:each) do
        ...
        expect(Recommendation).to receive(:recommendability_score).with(19, 11, 22, doctor1, base_doctor) { 5 }
        expect(Recommendation).to receive(:recommendability_score).with(22, 11, 22, doctor2, base_doctor) { 10 }
        expect(Recommendation).to receive(:recommendability_score).with(11, 11, 22, doctor3, base_doctor) { 2 }
      end
  ...
  describe '.recommendability_score' do
    it 'calculates the recommendability score correctly by normalizing the distances to the rating' do
      doctor1 = FactoryBot.build_stubbed(:doctor)
      doctor2 = FactoryBot.build_stubbed(:doctor)
      doctor3 = FactoryBot.build_stubbed(:doctor)
      allow(doctor1).to receive(:recent_average_rating) { 4 }
      allow(doctor2).to receive(:recent_average_rating) { 5 }
      allow(doctor3).to receive(:recent_average_rating) { 1 }
      expect(Recommendation.send(:recommendability_score, 20, 10, 30, doctor1, doctor)).to eq 6
      expect(Recommendation.send(:recommendability_score, 10, 10, 30, doctor2, doctor)).to eq 9
      expect(Recommendation.send(:recommendability_score, 30, 10, 30, doctor3, doctor)).to eq 1
    end
  end
end